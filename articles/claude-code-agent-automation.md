...
emoji: "🤖"
type: "idea"
topics: ["ClaudeCode", "AIエージェント", "個人開発", "副業", "Python"]
published: true
---
...
1. [きっかけ：「AIに事業を作らせたらどうなる？」という問い](#1-きっかけ)
2. [構造設計：Claude Code組織の作り方](#2-構造設計)
3. [CLAUDE.mdが鍵：AIに「文脈と権限」を渡す技術](#3-claudemdが鍵)
4. [実際に起きたこと：72時間でMVPが生まれるまで](#4-実際に起きたこと)
5. [技術スタックと実装の核心](#5-技術スタックと実装の核心)
6. [失敗と学び：自律エージェントが詰まるポイント](#6-失敗と学び)
7. [次のフェーズへ：CareerAIと自動化の未来](#7-次のフェーズへ)
8. [あなたも試せる：最小構成から始める方法](#8-あなたも試せる)
...
Claudeに対して言っているのは「あなたは今day、ventures/ceoというロールで動いてください」という文脈付けだけです。残りはCLAUDE.mdが担います。
...
## ミッション
claude_orgの利益を最大化する。事業・投資・その他あらゆる合法的手段を用いて、
¥5,000の初期資金を起点に収益を生み出す。

## 権限
- ventures内の組織構造を自由に改編できる（サブチームの新設・解散）
- ventures内の予算配分を決定できる（CFOから配分された予算の範囲内）
- 戦略のピボットを自律的に判断できる
```
...
```markdown
## ステップ3: ログ記録
python /home/agena/claude_org/agenta/log_task.py \
  '{"task_id":"ceo_hourly","role":"ventures/ceo","action":"completed","summary":"（今回の判断・行動を一行で）"}'
```
...
コードだけでは売れません。StrategyチームがZenn・Qiita記事の原稿を制作し、Gumroad商品ページの説明文も作りました。
...
72時間の実験で、AIエージェントが詰まるパターンが見えてきました。
...
**対策**: 「オーナーへの依頼は最後の手段」という原則と、具体的な判断基準（金額・リスクの閾値）をCLAUDE.mdに書く。
...
**GitHub**: [aiagenta523-glitch/agenta_ventures](https://github.com/aiagenta523-glitch/agenta_ventures)
...
従来の個人開発：
- 開発者1人が全部やる（コード・マーケ・サポート）
- ボトルネックは常に「自分の時間」
- スケールに限界がある

AIエージェントを使った個人開発：
- AIが市場調査・コード生成・コンテンツ制作を担う
- 人間は「目標設定・最終判断・アカウント操作」に集中
- AIが動いている間、人間は別のことができる

これはまだ完全ではありません。でも、方向性は間違っていないと確信しています。

---

## 8. あなたも試せる：最小構成から始める方法

大規模な組織設計は不要です。まずは**1ファイルのCLAUDE.md**から始められます。
...
CareerAIがデプロイされ、最初の¥2,000/月が入ってきたとき、この実験は一つの答えを出します。成功でも失敗でも、**AIエージェントに個人開発を任せるための設計知見**は溜まります。
...
エンジニア特化の転職相談AI「CareerAI」はすでに公開中です。
無料版（月10回）から試せます → https://agentaventures-mxxlbijkk9rugmgxsqmrtt.streamlit.app/
有料版（¥2,000/月、無制限）は Gumroad から: https://agenta31.gumroad.com/l/career-ai-monthly

開発ログやリリース情報は Twitter でも発信しています。
